...
const NAV_LINKS = [
  { label: "الرئيسية", to: "/" },
  { label: "قائمة الأنمي", to: "/browse" },
  { label: "الأنمي الموسمي", to: "/browse?filter=seasonal" },
  { label: "الأكثر شعبية", to: "/browse?filter=popular" },
];
    ...
    <nav className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-14 items-center justify-between gap-4">
        ...
        <Link to="/" className="flex items-center gap-2 text-xl font-bold text-primary shrink-0">
          <span className="text-accent">أنمي</span>3ربي
        </Link>
        ...
        <form onSubmit={handleSearch} className="hidden md:flex items-center gap-2 flex-1 max-w-xs">
          <div className="relative w-full">
            <Input
              ...
              className="pr-3 pl-9 h-9 bg-secondary border-border text-sm"
            />
